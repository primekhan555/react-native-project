...
import { View, Text, StatusBar, TouchableOpacity, TouchableHighlight, Image, AsyncStorage, StyleSheet } from 'react-native';
...
export default class PersonalInfo extends Component {
    ...
    componentDidMount() {
        AsyncStorage.getItem('CNIC', (err, result) => {
            ...
            return fetch(join)
                ...
                .then(() => {
                    const url = 'https://0a50e7d4.ngrok.io/api/PatientPic/';
                    var nicc = result;
                    ...
                    return fetch(joins)
                        .then((response) => response.json())
                        .then((responseJson) => {
                            const pic = responseJson.patientPicture
                            if (pic != null) {
                                this.setState({
                                    imageUri: pic,
                                })
    ...
    render() {
        ...
        return (
            <View style={{ flex: 1 }}>
                <StatusBar backgroundColor='#ff6666' barStyle='default' hidden={false} translucent={false} />
                <View style={{
                    height: 140,
                    width: '100%',
                                ...
                                ImagePicker.openPicker({
                                    ...
                                    return fetch(joining)
                                        ...
                                        .then((responceStatus) => {
                                            ...
                                            var cnic =this.state.gCNIC;
                                            const fullOwner = owner.concat(cnic);
                                            console.log("fullowner"+fullOwner)
                                            let block = {
                                                "$class": "org.com.mediblocking.PatientPic",
                                                "pictureId": CNICS,
                                                "nicNumPatient": CNICS,
                                                "patientPicture": this.state.imageUri,
                                                "patientP": fullOwner,
                                            }
                                            if (responceStatus == 200 || responceStatus == 304) {
                                                const puturl = 'https://0a50e7d4.ngrok.io/api/PatientPic/';
                                                ...
                                                return fetch('https://0a50e7d4.ngrok.io/api/PatientPic/', {
                                                    method: 'POST',
                                                    headers: {
                                                        Accept: 'application/json',
                                                        'Content-Type': 'application/json',
